...
const CONTRACT_LOCKED_FIELDS =
	typeof LOCKED_CONTRACT_FIELDS !== "undefined"
...
function populateDropdowns() {
	...
	huurderSelect.innerHTML = `<option value="">Selecteer huurder</option>${huurders
		.map(
	...
	tbody.innerHTML = filteredContracten
		.map((contract) => {
            ...
            <tr onclick="viewContractDetail('${contract.id}')" style="cursor: pointer;">
				...
				<td>€${Number.parseFloat(contract.huurprijs).toLocaleString("nl-NL")}</td>
...
addContractBtn.addEventListener("click", () => {
	...
	// Ensure all fields are unlocked for new contracts
	...
	const warning = document.getElementById("contractFaseWarning");
...
contractForm.addEventListener("submit", async (e) => {
	...
	const contractData = {
		...
		borg: Number.parseFloat(document.getElementById("borg").value) || 0,
		betalingsdatum:
			Number.parseInt(document.getElementById("betalingsdatum").value) || 1,
	...
	const existingContract = contractId
		? contracten.find((contract) => contract.id === contractId)
		: null;

	if (existingContract && typeof isContractMutable === "function") {
		if (!isContractMutable(existingContract)) {
			for (const field of CONTRACT_LOCKED_FIELDS) {
				contractData[field] = existingContract[field];
			}
		}
	}
	...
	try {
		...
		// Check for existing active contract on same pand (prevent overbooking)
		if (!contractId) {
			const existingContracts = await dbGetAll("contracten");
			const activeOnPand = existingContracts.filter(
				(c) =>
					c.pandId === contractData.pandId &&
					new Date(c.einddatum) > new Date() &&
					c.contractFase === "actief",
			);
			if (activeOnPand.length > 0) {
				hideLoading();
				const confirmed = await showConfirm(
					"Er is al een actief contract voor dit pand. Wilt u toch doorgaan?",
				);
				if (!confirmed) return;
				showLoading("Contract opslaan...");
			}
		}
...
async function editContract(id) {
	...
	// Enforce read-only fields for non-concept contracts
	const isNonConcept =
		contract.contractFase && contract.contractFase !== "concept";
	for (const fieldId of CONTRACT_LOCKED_FIELDS) {
		const el = document.getElementById(fieldId);
		if (el) {
			el.disabled = isNonConcept;
			el.title = isNonConcept
				? "Dit veld kan niet worden gewijzigd buiten conceptstatus"
				: "";
		}
	}

	// Show warning for non-concept contracts
	let warning = document.getElementById("contractFaseWarning");
	if (!warning) {
		warning = document.createElement("div");
		warning.id = "contractFaseWarning";
		warning.style.cssText =
			"padding: 10px 14px; margin-bottom: 12px; border-radius: 8px; font-size: 13px; display: none;";
	...
	if (isNonConcept) {
		...
		warning.textContent =
			"Dit contract is niet meer in conceptstatus. Alleen fase, einddatum, voorwaarden en referentie kunnen worden gewijzigd.";
	} else {
