...
function getStoredAzureConfig() {
	try {
		return JSON.parse(localStorage.getItem("azureConfig") || "null") || {};
	} catch (error) {
		console.warn("Failed to parse azureConfig from localStorage", error);
		return {};
	}
}

function getMsalAuthConfig() {
	const azureConfig = getStoredAzureConfig();
	const clientId = azureConfig.clientId || "";
	const tenantId = azureConfig.tenantId || "";

	if (!clientId || !tenantId) {
		throw new Error(
			"Azure AD is nog niet geconfigureerd. Vul Client ID en Tenant ID in via Admin > Azure AD.",
		);
	}
...
function getUserRole(account) {
	...
	if (account.idTokenClaims?.groups) {
		...
		const azureConfig = getStoredAzureConfig();
		const ADMIN_GROUP_ID = azureConfig.adminGroupId;
		const MANAGER_GROUP_ID = azureConfig.managerGroupId;

		if (ADMIN_GROUP_ID && groups.includes(ADMIN_GROUP_ID)) {
			return USER_ROLES.ADMIN;
		}
		if (MANAGER_GROUP_ID && groups.includes(MANAGER_GROUP_ID)) {
			return USER_ROLES.MANAGER;
		}
	}

	// Option 2: Check custom extension attributes
	if (account.idTokenClaims?.extension_Role) {
